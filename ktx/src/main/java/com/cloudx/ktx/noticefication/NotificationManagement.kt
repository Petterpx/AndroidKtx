package com.cloudx.ktx.noticefication
...
fun isChannelAvailable(chatId: String, obj: () -> Boolean = { true }) {
    manager.getNotificationChannel(chatId)?.apply {
...
fun sendNoticeMessage(
    ...
    objO: (NotificationCompat.Builder) -> Notification = { builder ->
        builder.build()
    },
    obj: (Notification.Builder) -> Notification = { builder ->
        builder.build()
    }
) {
    noticeMessageBean?.apply {
        ...
        if (isSdkO) {
            ...
            manager.notify(notificationId?: random.nextInt(), objO(sendNoticeO))
        } else {
            val sendNotice = sendNotice(title, content, smallIcon)

            manager.notify(notificationId?: random.nextInt(), obj(sendNotice))
        }
    }
}
